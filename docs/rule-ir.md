...
| `semantic_rubric` | `token_budget` | `32000` | Optional positive integer overriding the per-rule token budget for the narrowed affected-context assembly path (#281, S5). Applies only when a multi-file effective set (scope ∩ candidate) reaches `llm-rubric`: the backend assembles the files into one prompt under this budget and truncates deterministically (lexicographic prefix) when the set is over budget, naming omitted files in `truncation_warning` evidence. Precedence: `params.token_budget` → dotenv `GATE_KEEPER_TOKEN_BUDGET` → compiled-in `32000`. A malformed / non-positive value is ignored (falls through), never an error. No effect on single-target rules or the literal `params.targets` path. See [`docs/design/multi-target.md`](design/multi-target.md) §2.3 / §9.12. |
...
| Non-empty effective set | Rule runs against the intersection; a `scope_effective_set` evidence record (scope / candidate / effective sizes + effective paths) rides on the rule's normal verdict. A single-file effective set dispatches as a single target; a multi-file set aggregates on the `filesystem` backend and, on `llm-rubric`, is assembled into one narrowed affected-context prompt under a token budget (#281, S5 — see `token_budget` above and [`docs/design/multi-target.md`](design/multi-target.md) §9.12). |
